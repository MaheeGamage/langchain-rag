...
of exploring a new frontier of the physical sciences, what we might call thecomplexity
frontier or theentanglement frontier. This new frontier, while different than the frontier
...
of these principles is the idea ofquantum entanglement. Entanglement is the word we use
...
callqubits. A qubit can be realized physically in many different ways. It can be carried
...
It isverychallenging to build a quantum system that satisfies all of these desiderata. It
...
this impending new era, so I made up a word:NISQ. This stands forNoisy Intermediate-
...
we care about. We also care about thequalityof the qubits, and in particular the accuracy
...
superconducting circuits [15], the error rate per gate for two-qubit gates is above the_._ 1 %
...
measurement error probability is about1%for superconducting qubits, and much better
...
(^1) 72-qubit and 50-qubit devices, based on superconducting circuits, have been announced recently by
Google and IBM, respectively.
(^2) This “milestone” is actually a bit fuzzy, in several respects. In particular, the resources needed in a
...
certifiablyrandom bits, even if one distrusts the equipment used for this purpose —
...
improve.
A few years ago I spoke enthusiastically aboutquantum supremacyas an impending
...
we might formulate _m_ constraints on _n_ bits, and seek an _n_ -bit string which solves as many
of the _m_ constraints as possible. We may say we have solved this optimization problem
exactly if we can find the maximal number _k_ of constraints that can be satisfied simulta-
neously, and that we have an approximate solution if we can guarantee that the maximal
number of satisfiable constraints is at least _k_ ′for some _k_ ′ _< k_ ; theapproximation ratio
_k_ ′ _/k_ is a measure of the quality of our approximate solution.
...
quantum processor to prepare an _n_ -qubit state, then measure all the qubits and process
the measurement outcomes using a classical optimizer; this classical optimizer instructs
the quantum processor to alter slightly how the _n_ -qubit state is prepared. This cycle is
...
Naively one might expect that, for a generic circuit with _G_ gates, a single fault anywhere
in the circuit could cause the computation to fail; if so, then we cannot execute the circuit
reliably if the error rate per gate is much larger than 1 _/G_. However, at least for some
...
with _N_ components in justlog _N_ qubits. But typical proposals for quantum machine
...
the state, and we can acquire no more thanlog _N_ bits of classical information in a single-
shot measurement oflog _N_ qubits.
...
ago) takes as input a succinct representation of a very large _N_ × _N_ matrix _A_ , which has
to be sufficiently sparse and well conditioned, and also as input an _N_ -component vector
| _b_ 〉, encoded in QRAM as a quantum state oflog _N_ qubits. The output of the quantum
algorithm is (with a small error) the quantum state| _A_ −^1 _b_ 〉, the result of applying the
inverse of the matrix _A_ to the input vector| _b_ 〉. This quantum algorithm runs in time
_O_ (log _N_ ), an exponential speedup relative to classical matrix inversion (for fixed error and
for fixed sparsity and condition number of _A_ ).
However, aside from the caveat that the matrix _A_ must be sparse and well conditioned,
note that both the input vector| _b_ 〉and the output vector| _A_ −^1 _b_ 〉are quantum states. Once
we have the output state, we can perform measurements to learn features of the output
vector _A_ −^1 _b_ , and by repeating the scheme many times we can learn about the output vector
...
data into QRAM might be avoided by computing| _b_ 〉on a quantum computer rather than
...
the best currently known classical algorithm for a useful task: making high-valuerecom-
...
customers.
To formulate a simplified version of the problem, consider _m_ customers and _n_ products,
and let _P_ be a binary _m_ × _n_ preference matrixsuch that _Pai_ = 1if customer _a_ likes product
_i_ and _Pai_ = 0if customer _a_ dislikes product _i_. In practice we might have _m_ ≈ 108 users and
_n_ ≈ 106 products, but the rank _k_ of the matrix is relatively small, perhaps _k_ ≈ 100. This
...
offline, a low rank approximation to the preference matrix _P_ is constructed. In the second
...
stage that admits a quantum speedup. The quantum runtime is _O_ (poly( _k_ )polylog( _mn_ )),
while the best classical algorithm known requires timepoly( _mn_ )to return a high-value
...
Instead it samples efficiently from the low-rank approximation to _P_.
...
tum recommendation systems (returning a high-quality recommendation in polylog( _mn_ )
...
inequality constraints. Specifically, given as input the _m_ +1 _N_ × _N_ Hermitian matrices
{ _C,A_ 1 _,A_ 2 _,...,Am_ } and real numbers{ _b_ 1 _,b_ 2 _,...,bm_ }, the problem is to find a posi-
tive semidefinite _N_ × _N_ matrix _X_ that maximizestr( _CX_ )subject to the _m_ constraints
tr( _AiX_ )≤ _bi_. Convex optimization problems of this type have widespread applications,
and can be solved classically inpoly( _m,N_ )time.
A recently discovered quantum algorithm [46, 47] finds an approximate solution to
the problem in timepolylog( _N_ ), an exponential speedup. In the quantum algorithm, the
output is a quantum state, a density operator _ρ_ which approximates the optimal matrix
_X_. This quantum state can then be measured to extract information about _X_ , and the
procedure can be repeated many times to learn about _X_ is more detail.
...
(^3) Note added: After this paper was submitted, Tang [45] exhibited a “quantum-inspired” classical algo-
rithm which returns a high-value recommendation in runtime _O_ (poly( _k_ )polylog( _mn_ )); thus the quantum
...
Classical computers are especially bad at simulatingquantum dynamics— that is, pre-
...
analog quantum simulatorwe mean a system with many qubits whose dynamics resembles
the dynamics of a model system we are trying to study and understand. In contrast, a
digital quantum simulatoris a gate-based universal quantum computer which can be used
...
tures that physicists calluniversal, properties which are relatively robust with respect to
...
[9] R. P. Feynman, Simulating physics with computers, Int. J. Theor. Physics 21, 467-
(1982).
```
...
[44] I. Kerenidis and A. Prakash, Quantum recommendation systems, arXiv:1603.
(2016).
